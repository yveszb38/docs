...
As mentioned earlier, {{ $names.company.lower }} HostOS defaults to using ethernet for internet connectivity, so if you connect an active ethernet cable to your device and all the [network requirements](#network-requirements) are satisfied, your device should simply just connect to dashboard.{{ $names.domain }} and you should be able to push code to it.
...
To get started with a cellular connection have a look at our blog post ["A guide to cellular connectivity on {{ $names.company.lower }} devices"](https://{{ $names.domain }}/blog/cellular-connectivity/) and the corresponding [github repository]({{ $links.githubProjects }}/cellular-modem.git).
...
[wifi-connect-blog]:https://{{ $names.domain }}/blog/resin-wifi-connect/
